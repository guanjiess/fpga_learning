...
(
	...
	output						o_uart_clk		,	// used for simulation
	output						o_uart_rst			
);

localparam 			CLK_DIV_CNT = P_SYS_CLK * 1000000 / P_BAUDRATE; // calculate the frequency div number

wire				w_uart_baud_clk;
wire				w_uart_baud_clk_rst;
assign 				o_uart_clk			=	w_uart_baud_clk		;
assign				o_uart_rst			=	w_uart_baud_clk_rst	;

// frequency divider, output frequency should be the same as baudrate
CLK_DIV_module #(
	.P_DIV_CYCLE		(CLK_DIV_CNT) 
)
CLK_DIV_module_u0 
(
	.i_clk		(	i_clk),
